...
//Adicionado como funcao para ser reutilizada em ordenarEntidades
...
void ordenarEntidades(Entidade** e, int quantidade){
	...
	//buscar jogador
	if(e[0]->tipo != ENTIDADE_JOGADOR){
		for(int i=0;i<quantidade;i++){
			if(e[i]->tipo == ENTIDADE_JOGADOR){
	...
	//O algoritmo usado e o ICBICS.
	//Referencia: Fung, S.P.Y, Is this the simplest (and most surprising) sorting algorithm ever?, 2021, disponivel em https://arxiv.org/html/2110.01111v1
	for(int i=1;i<quantidade;i++){
		for(int j=1;j<quantidade;j++){
			if(distancia(e[0],e[j-1])>distancia(e[0],e[j])){
				temp = e[j-1];
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
		...
		//Ordenar vetorEntidades em relacao a distancia ate o jogador
		ordenarEntidades(vetorEntidades, totalEntidades);
