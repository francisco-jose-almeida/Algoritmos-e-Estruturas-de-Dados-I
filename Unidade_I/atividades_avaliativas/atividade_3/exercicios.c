...
Item *criarItens(int quantidade) {
    ...
    for (int i = 0; i < quantidade; i++) {
        ...
        if (it->tipo == ITEM_ARMA) {
			...
			it->dados.escudo.defesa = GetRandomValue(1, 3);
		}else{
...
void aplicarItem(Jogador *j, Item *item) {
    switch (item->tipo) {
        case ITEM_ARMA:
            ...
            break;
		case ITEM_ESCUDO:
			j->armadura += item->dados.escudo.defesa;
			break;
		case ITEM_POCAO:
...
void desenharItem(Item *item) {
    ...
    Color cor = (item->tipo == ITEM_ARMA) ? RED : (item->tipo == ITEM_ESCUDO) ? ORANGE : GREEN;
...
int main(void) {
    ...
    Jogador jogador = { { LARGURA_JANELA / 2.0f, ALTURA_JANELA / 2.0f }, RAIO_JOGADOR, 100, 1, 5.0f };
    ...
    while (!WindowShouldClose()) {
        ...
        BeginDrawing();
            ...
            DrawText(TextFormat("Vida: %d Armadura: %d  Dano: %.1f", jogador.vida, jogador.armadura, jogador.dano), 10, 10, 22, DARKGRAY);
