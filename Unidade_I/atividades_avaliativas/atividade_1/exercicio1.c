...
// Retorna uma instancia do tipo Bola
// *Reutilizei essa parte da funcao criarBolas para um codigo mais limpo
void fillBola(Bola *b){
...
int main(void) {
	...
	//Modificar quantidade de bolas com teclado:
	if(IsKeyPressed(KEY_SPACE)){
		quantidadeBolas++;
		bolas = (Bola *) realloc(bolas, quantidadeBolas*sizeof(Bola));
		if(bolas == NULL){exit(0);};
        ...
        BeginDrawing();
            ...
            DrawText("Matriz (int**) e vetor de structs (Bola*) alocados com malloc",
                     10, 10, 18, WHITE);
	    DrawText("Precione ESPACO ou BACKSPACE para alterar a quantidade",
                     10, 30, 18, WHITE);
