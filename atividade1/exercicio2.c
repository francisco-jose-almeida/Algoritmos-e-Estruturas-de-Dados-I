...
int **criarMatriz(int linhas, int colunas) {
    ...
    for (int i = 0; i < linhas; i++) {
        ...
        for (int j = 0; j < colunas; j++) {
            ...
            matriz[i][j] = 0;//GetRandomValue(0, 1);
        }
    }
    return matriz;
}
...
//Verifica se uma das bolas esta em uma celula nao visitada, ou seja grade[i][j]==0,
//caso afirmativo, grade[i][j]=1;
void checkBoard(Bola* bs, int** matriz,int quantidade,int* contador){
	for (int index=0;index<quantidade;index++){
		int j = (int) ((bs+index)->pos.x/TAM_CELULA);
		int i = (int) ((bs+index)->pos.y/TAM_CELULA);
...
int main(void) {
	...
	checkBoard(bolas, grade, quantidadeBolas,&celulas_visitadas);
        ...
        BeginDrawing();
            ...
            DrawText("Matriz (int**) e vetor de structs (Bola*) alocados com malloc",
                     10, 10, 18, WHITE);
            DrawText(TextFormat("Espacos Visitados: %d",celulas_visitadas),
                     10, 50, 18, WHITE);
